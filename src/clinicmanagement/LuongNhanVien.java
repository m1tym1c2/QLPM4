...
public class LuongNhanVien extends javax.swing.JFrame {
    ...
    private void InitData() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                row.add(String.format("%,d", rs.getInt("LuongCB")));
                row.add(String.valueOf(rs.getFloat("HeSo")));
                row.add(String.format("%,d", rs.getInt("TienThuong")));
                row.add(String.format("%,d", (int) (rs.getInt("LuongCB") * rs.getFloat("HeSo") + rs.getInt("TienThuong"))));
                model.getRowCount();
                model.addRow(row);
            }
        } catch (Exception e) {
        ...
        for (int i = 0; i < table.getRowCount(); i++) {
            ...
            try {
                Number Amount = format.parse(table.getValueAt(i, 6) + "");
                sum += Amount.intValue();
            } catch (ParseException ex) {
                Logger.getLogger(LuongNhanVien.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
        jLabel6.setText(String.format("%,d",sum) + " VNĐ");
    }
    ...
    private void initComponents() {
        ...
        jPanel2.add(Tentaikhoan, new org.netbeans.lib.awtextra.AbsoluteConstraints(790, 20, -1, -1));
        ...
        jPanel2.add(Nutmuiten, new org.netbeans.lib.awtextra.AbsoluteConstraints(940, 20, 44, -1));
